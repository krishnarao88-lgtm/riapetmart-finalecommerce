...
export function LocalSeo() {
  ...
  return (
    <section aria-labelledby="local-heading" className="mx-auto max-w-6xl px-4 pb-14">
      <div className="grid gap-6 rounded-3xl border-2 border-choc bg-surface p-6 sm:p-8 md:grid-cols-[1.6fr_1fr]">
        ...
        <ul className="grid content-start gap-2">
          {links.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
                className="flex items-center justify-between rounded-2xl border-2 border-choc/20 px-4 py-3 font-bold text-choc hover:border-terracotta"
              >
                {link.label}
